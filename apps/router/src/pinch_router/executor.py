...
def minimax_messages(trace: TraceInput) -> list[dict[str, str]]:
    """Serialize the demo trace into MiniMax's simple text chat message form."""
...
class MiniMaxExecutor:
    ...
    def execute(self, trace: TraceInput) -> ModelExecution:
        ...
        if not api_key:
            raise RuntimeError("Missing MiniMax API key in execution_profiles.minimax_m3.api_key")
        payload = {
            "model": self.profile.model_name,
            "messages": minimax_messages(trace),
        }
        request = Request(
        ...
        try:
            ...
            raise RuntimeError(f"MiniMax request failed: {error.reason}") from error
        try:
            message = body["choices"][0]["message"]
        except (KeyError, IndexError, TypeError) as error:
            raise RuntimeError(f"Unexpected MiniMax response: {body}") from error
